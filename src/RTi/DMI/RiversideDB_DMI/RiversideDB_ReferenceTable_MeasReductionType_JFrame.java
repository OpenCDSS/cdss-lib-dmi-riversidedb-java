...
//------------------------------------------------------------------------------
// RiversideDB_ReferenceTable_MeasReductionType_JFrame - Frame extending from
//    RiversideDB_ReferenceTable_Abstract_JFrame and containing the editor for
//    the RiversideDB table MeasReduction.
//------------------------------------------------------------------------------
...
the editor for the RiversideDB table MeasReductionType.
*/
public class RiversideDB_ReferenceTable_MeasReductionType_JFrame extends
	RiversideDB_ReferenceTable_Abstract_JFrame {

/**
Class name.
*/
public final String CLASS = "RiversideDB_ReferenceTable_MeasReductionType_JFrame";
...
*/
public RiversideDB_ReferenceTable_MeasReductionType_JFrame(
		RiversideDB_DMI dmi,
	...
	_title = _title + "Meas Reduction Type";
...
{
// Creates a blank record object for the MeasReductionType table, marks it
	// as a new, and inserts it into the tabl.
	RiversideDB_MeasReductionType mrt = new RiversideDB_MeasReductionType();
	mrt.setOriginal   ( null );
	_worksheet.addRow (  mrt );
...
protected DMIDataObject[] checkForKeyChange(DMIDataObject ddo)
{
	...
	RiversideDB_MeasReductionType mrt = (RiversideDB_MeasReductionType)
					ddo;
	RiversideDB_MeasReductionType mrt2 = (RiversideDB_MeasReductionType)
					mrt.getOriginal();
	mrt2.setOriginal(mrt2.cloneSelf());
	if (mrt.getType().trim().equals(mrt2.getType().trim())) {
		return null;
	}
	obs[0] = mrt;
	obs[1] = mrt2;
...
protected boolean dataObjectsEqual( DMIDataObject d1, DMIDataObject d2 )
{
	...
	eq = (	(RiversideDB_MeasReductionType) d1 ).equals(
		(RiversideDB_MeasReductionType) d2 );
...
throws Exception
{
	...
	RiversideDB_MeasReductionType mrt  = (RiversideDB_MeasReductionType)
					ddo;
	RiversideDB_MeasReductionType mrt0 = (RiversideDB_MeasReductionType)
					mrt.getOriginal();
	RiversideDB_MeasReductionType mrtr =
		_dmi.readMeasReductionTypeForType( mrt0.getType() );

	if ( shouldDelete(
		mrt, mrt0, mrtr, "Type", mrt.getType() ) ) {
		_dmi.deleteMeasReductionTypeForType( mrt0.getType() );
	}
}
...
protected void informWindowManager()
{
	_windowManager.closeWindow(
		_windowManager.WINDOW_REF_MEAS_REDUCTION_TYPE );
}
...
protected void readData()
{
	...
	try {
		_results = _dmi.readMeasReductionTypeList();
	}
	catch ( Exception e ) {
...
protected void setupGUI()
{
	...
	try {
		_model = new 
			RiversideDB_ReferenceTable_MeasReductionType_TableModel (
...
throws Exception
{
	...
	RiversideDB_MeasReductionType mrt0 = (RiversideDB_MeasReductionType)
					mrt.getOriginal();
	RiversideDB_MeasReductionType mrtr =
		_dmi.readMeasReductionTypeForType( mrt.getType() );

	if ( shouldWrite(
		mrt, mrt0, mrtr, "Type", mrt.getType() ) ) {
		_dmi.writeMeasReductionType( mrt );
	}
}
