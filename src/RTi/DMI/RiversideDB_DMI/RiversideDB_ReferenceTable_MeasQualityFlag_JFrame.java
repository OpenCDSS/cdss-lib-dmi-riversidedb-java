...
//------------------------------------------------------------------------------
// RiversideDB_ReferenceTable_MeasQualityFlag_JFrame - Frame extending from
//    RiversideDB_ReferenceTable_Abstract_JFrame and containing the editor for
//    the RiversideDB table MeasQuality.
//------------------------------------------------------------------------------
...
the editor for the RiversideDB table MeasQualityFlag.
*/
public class RiversideDB_ReferenceTable_MeasQualityFlag_JFrame extends
	RiversideDB_ReferenceTable_Abstract_JFrame {

/**
Class name.
*/
public final String CLASS = "RiversideDB_ReferenceTable_MeasQualityFlag_JFrame";
...
*/
public RiversideDB_ReferenceTable_MeasQualityFlag_JFrame(
		RiversideDB_DMI dmi,
	...
	_title = _title + "Meas Quality Flag";
...
{
	// Creates a blank record object for the MeasQualityFlag table, marks it
	// as a new, and inserts it into the tabl.
	RiversideDB_MeasQualityFlag mqm = new RiversideDB_MeasQualityFlag();
	mqm.setOriginal  ( null );
	_worksheet.addRow(  mqm );
...
protected DMIDataObject[] checkForKeyChange(DMIDataObject ddo)
{
	...
	RiversideDB_MeasQualityFlag mqf = (RiversideDB_MeasQualityFlag)
					ddo;
	RiversideDB_MeasQualityFlag mqf2 = (RiversideDB_MeasQualityFlag)
					mqf.getOriginal();
	mqf2.setOriginal(mqf2.cloneSelf());
	if (mqf.getQuality_flag().trim().equals(mqf2.getQuality_flag().trim())) {
		return null;
	}
	obs[0] = mqf;
	obs[1] = mqf2;
...
protected boolean dataObjectsEqual( DMIDataObject d1, DMIDataObject d2 )
{
	...
	eq = (	(RiversideDB_MeasQualityFlag) d1 ).equals(
		(RiversideDB_MeasQualityFlag) d2 );
...
throws Exception
{
	...
	RiversideDB_MeasQualityFlag mqf  = (RiversideDB_MeasQualityFlag)
						ddo;
	RiversideDB_MeasQualityFlag mqf0 = (RiversideDB_MeasQualityFlag)
						mqf.getOriginal();
	RiversideDB_MeasQualityFlag mqfr =
		_dmi.readMeasQualityFlagForQuality_flag( mqf0.getQuality_flag() );

	if ( shouldDelete(
		mqf, mqf0, mqfr, "Quality Flag", mqf.getQuality_flag() ) ) {
		_dmi.deleteMeasQualityFlagForQuality_flag( mqf0.getQuality_flag() );
	}
}
...
{
	_worksheet.addRow(new RiversideDB_MeasQualityFlag());
...
protected void informWindowManager()
{
	_windowManager.closeWindow(
		_windowManager.WINDOW_REF_MEAS_QUALITY_FLAG );
}
...
protected void readData()
{
	...
	try {
		_results = _dmi.readMeasQualityFlagList();
	}
	catch ( Exception e ) {
...
protected void setupGUI()
{
	...
	try {
		_model = new 
			RiversideDB_ReferenceTable_MeasQualityFlag_TableModel (
...
throws Exception
{
	...
	RiversideDB_MeasQualityFlag mqf0 = (RiversideDB_MeasQualityFlag)
					mqf.getOriginal();
	RiversideDB_MeasQualityFlag mqfr =
		_dmi.readMeasQualityFlagForQuality_flag( mqf.getQuality_flag() );

	if ( shouldWrite(
		mqf, mqf0, mqfr, "Quality Flag", mqf.getQuality_flag() ) ) {
		_dmi.writeMeasQualityFlag( mqf );
	}
}
