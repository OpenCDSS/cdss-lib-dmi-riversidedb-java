...
/**
Table model used for displaying thw data editor for the table MeasCreateMethod.
This class extends from RiversideDB_ReferenceTable_Abstract_TableModel.
*/
...
public void backupData()
{
	for (int i = 0; i < _rows; i++) {
		RiversideDB_MeasCreateMethod m1 =
			(RiversideDB_MeasCreateMethod) _data.elementAt(i);
...
protected String checkTableField( String s, int row )
{
	...
	for (int i = 0; i < _rows; i++) {
		if (i != row) {
			d = (RiversideDB_MeasCreateMethod)_data.elementAt(i);
...
{
	RiversideDB_MeasCreateMethod m =
		(RiversideDB_MeasCreateMethod)_data.elementAt(row);
	switch (column) {
...
protected Object setTableValueAt( Object value, int row, int column )
{
	RiversideDB_MeasCreateMethod m =
		(RiversideDB_MeasCreateMethod)_data.elementAt(row);
	RiversideDB_MeasCreateMethod m0 =
		(RiversideDB_MeasCreateMethod)m.getOriginal();
	String s;
	switch (column) {
		case COL_MEAS_CREATE_METHOD_METHOD:
