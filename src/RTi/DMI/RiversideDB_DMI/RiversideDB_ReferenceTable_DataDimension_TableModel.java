...
//------------------------------------------------------------------------------
// RiversideDB_ReferenceTable_DataDimension_TableModel - Table model used for
// 	displaying thw data editor for the reference table DataDimension.
...
//		   			RiversideDB_ReferenceTable_JFrame
// 2004-12-16 Luiz Teixeira		Split this class into:
//					RiversideDB_ReferenceTable_EditorBase_TableModel
//					and
...
/**
Table model used for displaying thw data editor for the table DataDimension.
This class extends from RiversideDB_ReferenceTable_Abstract_TableModel.
*/
public class RiversideDB_ReferenceTable_DataDimension_TableModel
...
public final static int
	COL_DATA_DIMENSION_DIMENSION   = 0,
	COL_DATA_DIMENSION_DESCRIPTION = 1,
	NUMBER_OF_COLUMNS              = 2;
...
{
	RiversideDB_DataDimension current =
		(RiversideDB_DataDimension) currentObject;
	RiversideDB_DataDimension original =
		(RiversideDB_DataDimension) originalObject;
...
{
	switch ( column ) {	
		case COL_DATA_DIMENSION_DIMENSION:   return String.class;
		case COL_DATA_DIMENSION_DESCRIPTION: return String.class;
...
{
	switch ( column ) {	
		case COL_DATA_DIMENSION_DIMENSION:   return "Dimension";
		case COL_DATA_DIMENSION_DESCRIPTION: return "Description";
		default:                             return " ";
	}
}
...
public String[] getColumnToolTips()
{
	...
	tips[COL_DATA_DIMENSION_DIMENSION]   = "DataDimension.Dimension";
	tips[COL_DATA_DIMENSION_DESCRIPTION] = "DataDimension.Description";                                     	
	return tips;
}
...
public int[] getColumnWidths()
{
	...
	widths[COL_DATA_DIMENSION_DIMENSION]   = 15;
	widths[COL_DATA_DIMENSION_DESCRIPTION] = 35;
	return widths;
}
...
{
	switch (column) {	
		case COL_DATA_DIMENSION_DIMENSION:   return "%-20s";
		case COL_DATA_DIMENSION_DESCRIPTION: return "%-20s";
		default:			     return "%-20s";
	}
}
...
protected Object setTableValueAt( Object value, int row, int column )
{
	...
	switch ( column ) {
		case COL_DATA_DIMENSION_DIMENSION:
			...
			if ( !equal(s, d.getDimension()) ) {
				d.setDimension( s );
				checkTableDirty( d, d0 );
				valueChanged( row, column, value );
			}
			return (Object) s;
		case COL_DATA_DIMENSION_DESCRIPTION:
			s = readString( value, 50 );
			value = s;
			if ( !equal(s, d.getDescription()) ) {
				d.setDescription( s );
				checkTableDirty( d, d0 );
				valueChanged( row, column, value );
			}
