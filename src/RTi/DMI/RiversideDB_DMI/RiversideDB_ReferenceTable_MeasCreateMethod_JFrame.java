...
//------------------------------------------------------------------------------
// RiversideDB_ReferenceTable_MeasCreateMethod_JFrame - Frame extending from
//    RiversideDB_ReferenceTable_Abstract_JFrame and containing the editor for
//    the RiversideDB table MeasCreateMethod.
//------------------------------------------------------------------------------
...
the editor for the RiversideDB table MeasCreateMethod.
*/
public class RiversideDB_ReferenceTable_MeasCreateMethod_JFrame extends
	RiversideDB_ReferenceTable_Abstract_JFrame {

/**
Class name.
*/
public final String CLASS = "RiversideDB_ReferenceTable_MeasCreateMethod_JFrame";
...
*/
public RiversideDB_ReferenceTable_MeasCreateMethod_JFrame(
		RiversideDB_DMI dmi,
	...
	_title = _title + "Meas Create Method";
...
{
	// Creates a blank record object for the MeasCreateMethod table, marks
	// it as a new, and inserts it into the tabl.
	RiversideDB_MeasCreateMethod mcm = new RiversideDB_MeasCreateMethod();
	mcm.setOriginal   ( null );
	_worksheet.addRow (  mcm );
...
protected DMIDataObject[] checkForKeyChange(DMIDataObject ddo)
{
	...
	RiversideDB_MeasCreateMethod mcm = (RiversideDB_MeasCreateMethod)
					ddo;
	RiversideDB_MeasCreateMethod mcm2 = (RiversideDB_MeasCreateMethod)
					mcm.getOriginal();
	mcm2.setOriginal(mcm2.cloneSelf());
	if (mcm.getMethod().trim().equals(mcm2.getMethod().trim())) {
		return null;
	}
	obs[0] = mcm;
	obs[1] = mcm2;
...
protected boolean dataObjectsEqual( DMIDataObject d1, DMIDataObject d2 )
{
	boolean eq = false;
		...
		eq = (	(RiversideDB_MeasCreateMethod) d1 ).equals(
			(RiversideDB_MeasCreateMethod) d2 );
...
throws Exception
{
	...
	RiversideDB_MeasCreateMethod mcm  = (RiversideDB_MeasCreateMethod)
						ddo;
	RiversideDB_MeasCreateMethod mcm0 = (RiversideDB_MeasCreateMethod)
						mcm.getOriginal();
	RiversideDB_MeasCreateMethod mcmr =
		_dmi.readMeasCreateMethodForMethod( mcm0.getMethod() );

	if ( shouldDelete(
		mcm, mcm0, mcmr, "Method", mcm.getMethod() ) ) {
		_dmi.deleteMeasCreateMethodForMethod( mcm0.getMethod() );
	}
}
...
{
	_worksheet.addRow(new RiversideDB_MeasCreateMethod());
...
protected void informWindowManager()
{
	_windowManager.closeWindow(
		_windowManager.WINDOW_REF_MEAS_CREATE_METHOD );
}
...
protected void readData()
{
	...
	try {
		_results = _dmi.readMeasCreateMethodList();
	}
	catch ( Exception e ) {
...
protected void setupGUI()
{
	...
	try {
		_model = new 
			RiversideDB_ReferenceTable_MeasCreateMethod_TableModel (
...
throws Exception
{
	...
	RiversideDB_MeasCreateMethod mcm0 = (RiversideDB_MeasCreateMethod)
					mcm.getOriginal();
	RiversideDB_MeasCreateMethod mcmr =
		_dmi.readMeasCreateMethodForMethod( mcm.getMethod() );

	if ( shouldWrite(
		mcm, mcm0, mcmr, "Method", mcm.getMethod() ) ) {
		_dmi.writeMeasCreateMethod( mcm );
	}
}
