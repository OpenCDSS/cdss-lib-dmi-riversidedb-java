...
//------------------------------------------------------------------------------
// RiversideDB_ReferenceTable_DataSource_TableModel - Table model used for
// 	displaying thw data editor for the reference table DataSource.
...
/**
Table model used for displaying thw data editor for the table DataSource.
This class extends from RiversideDB_ReferenceTable_Abstract_TableModel.
*/
public class RiversideDB_ReferenceTable_DataSource_TableModel
...
public final static int
	COL_DATA_SOURCE_SOURCE_ABBREV = 0,
	COL_DATA_SOURCE_SOURCE_NAME   =	1,
	NUMBER_OF_COLUMNS             = 2;
...
{
	RiversideDB_DataSource current =
		(RiversideDB_DataSource) currentObject;
	RiversideDB_DataSource original =
		(RiversideDB_DataSource) originalObject;
...
{
	switch ( column ) {
		case COL_DATA_SOURCE_SOURCE_ABBREV: return String.class;
		case COL_DATA_SOURCE_SOURCE_NAME:   return String.class;
...
{
	switch ( column ) {	                                 
		case COL_DATA_SOURCE_SOURCE_ABBREV: return "Source\nAbbreviation";
		case COL_DATA_SOURCE_SOURCE_NAME:   return "\nSource Name"; 
		default:                            return " ";
	}
}
...
public String[] getColumnToolTips()
{
	...
	tips[COL_DATA_SOURCE_SOURCE_ABBREV] = "DataSource.Source_abbrev";
	tips[COL_DATA_SOURCE_SOURCE_NAME]   = "DataSource.Source_name";	
	return tips;
}
...
public int[] getColumnWidths()
{
	...
	widths[COL_DATA_SOURCE_SOURCE_ABBREV] =	10;
	widths[COL_DATA_SOURCE_SOURCE_NAME]   = 50;
	return widths;
}
...
{
	switch (column) {		
		case COL_DATA_SOURCE_SOURCE_ABBREV: return "%-20s";
		case COL_DATA_SOURCE_SOURCE_NAME:   return "%-20s";
		default:			    return "%-20s";
	}
}
...
protected Object getTableValueAt( int row, int column )
{
	...
	switch (column) {
		...
		case COL_DATA_SOURCE_SOURCE_NAME:
			return d.getSource_name();
		default:
			return " ";
	}
}
...
protected Object setTableValueAt( Object value, int row, int column )
{
	...
	switch (column) {
		case COL_DATA_SOURCE_SOURCE_ABBREV:
			...
			if (!equal(s, d.getSource_abbrev())) {
				d.setSource_abbrev(s);
