...
// Special properties for the diadvisor 504.
private  JTextField __props_tab_add_source_dir_JTextField  = null;
private  SimpleJButton  __props_tab_add_source_browse_JButton  = null;
private  JTextField __props_tab_add_source_file_JTextField = null;
private  SimpleJComboBox __props_tab_add_source_wild_JComboBox   = null;
...
public JPanel assemble_tab_properties( ) {
	...
	//JTextField for Retries - noneditable
 	...
 	// The following properties are used only by the diadvisor
 	// -------------------------------------------------------
 	//Select Source directory
	JLabel props_tab_add_source_dir_JLabel = new JLabel( "Additional Source Directory:");
	props_tab_add_source_dir_JLabel.setToolTipText(
	"Type in or browse for Import Product's additional source " +
	"directory (without file name)" );

	//type in directory
 	__props_tab_add_source_dir_JTextField = new JTextField( 15 );

	//button to browse to directory
 	__props_tab_add_source_browse_JButton = new SimpleJButton( "Browse", this );
 	__props_tab_add_source_browse_JButton.setToolTipText("Browse for directory");

	//Select source FILE iteself
	JLabel props_tab_add_source_file_JLabel = new JLabel( "Additional Source File(s):" );
	props_tab_add_source_file_JLabel.setToolTipText(
	"Type in file name for Import Product, using wildcards if needed.");

	//Type in  source file name
 	__props_tab_add_source_file_JTextField = new JTextField( 15  );

	//Use wildcard to get source file names
	JLabel props_tab_add_source_wild_JLabel = new JLabel( "Wildcards" );
	props_tab_add_source_wild_JLabel.setToolTipText("<HTML>Optional: "+
	"use wildcards to define imported file name. <BR>" +
	"Wildcards will be appended to file name. </html>");
...
//	"File column number for Table ID" );
	...
	if( __db_ImportProduct_type.equalsIgnoreCase( __type_diadvisor_str) ) {
		...
		//label for source directory
		JGUIUtil.addComponent( 
			props_JPanel, props_tab_add_source_dir_JLabel,
			0, ++y, 1, 1, 0.0, 0.0, __insets,
			GridBagConstraints.NONE,
			GridBagConstraints.EAST );
		...
		//text field to enter name of source directory
		JGUIUtil.addComponent( 
			props_JPanel, __props_tab_add_source_dir_JTextField,
			1, y, 1, 1, 0.0, 0.0, __insets,
			GridBagConstraints.NONE,
			GridBagConstraints.WEST );
		...
		//Button to browse for source directory name
		JGUIUtil.addComponent( 
			props_JPanel, __props_tab_add_source_browse_JButton,
			2, y, 1, 1, 0.0, 0.0, __insets,
			GridBagConstraints.NONE,
			GridBagConstraints.EAST );
		...
		//label for source file
		JGUIUtil.addComponent( 
			props_JPanel, props_tab_add_source_file_JLabel,
			0, ++y, 1, 1, 0.0, 0.0, __insets,
			GridBagConstraints.NONE,
			GridBagConstraints.EAST );
		...
		//text field to enter name of source file
		JGUIUtil.addComponent( 
			props_JPanel, __props_tab_add_source_file_JTextField,
			1, y, 1, 1, 0.0, 0.0, __insets,
			GridBagConstraints.NONE,
			GridBagConstraints.WEST );
		...
		//LABEL for ComboBox to select wildcard for source file
		JGUIUtil.addComponent( 
			props_JPanel, props_tab_add_source_wild_JLabel,
			2, y, 1, 1, 0.0, 0.0, __insets,
			GridBagConstraints.NONE,
			GridBagConstraints.EAST );
		...
		//ComboBox to select wildcard for source file
		JGUIUtil.addComponent( 
			props_JPanel, __props_tab_add_source_wild_JComboBox,
			3, y, 1, 1, 0.0, 0.0, __insets,
			GridBagConstraints.NONE,
			GridBagConstraints.WEST );
...
protected void checkRequiredInput() throws Exception {
	...
	if( __db_ImportProduct_type.indexOf( "TSLOOKUP" ) <0 &&
	    ( ( dest_dir == null ) || ( dest_dir.length() <= 0 ) ) ) {
		buffer.append( "Unable to update database " + 
	...
	*/
	...
	if ( table_size <= 0 && 
	     !__db_ImportProduct_type.equalsIgnoreCase( __type_tslookup_str) ) {
		buffer.append( "Unable to update database " + 
		...
		for ( int i=0; i< table_size; i++ ) {
			...
			//DMIUtil.MISSING_STRING ) ) ) {
			if ( (  __db_ImportProduct_type.indexOf( "SHEF" ) <0 )
			     && __db_ImportProduct_type.indexOf( "TSLOOKUP" ) <0
			     && ( DMIUtil.isMissing( ic.getExternal_table() ) ) ) {
			...
			if ( __db_ImportProduct_type.indexOf( "TSLOOKUP" ) <0 
				&& DMIUtil.isMissing( ic.getExternal_field() ) ) {
				//if ( __db_ImportProduct_type.equalsIgnoreCase( 
				//__type_shef_str) ) {
				if ( __db_ImportProduct_type.indexOf( 
				"SHEF" ) >= 0  ) {
...
protected List getListOfProperties( RiversideDB_ImportProduct ip ) {
	...
	if ( Message.isDebugOn ){
		Message.printDebug( 4, routine, "Property string for selected ImportProduct object is: \"" +
		tmp_props_str+ "\"." );
	}
...
protected void update_GUI_fields_propertiesTab( ) {
	...
	// Special fields for the diadvisor product property tab
	if ( __db_ImportProduct_type.equalsIgnoreCase( __type_diadvisor_str)) {
		
		//get additional source dir	
		String add_db_source_dir= null;
		add_db_source_dir = __db_RTi_ImportProduct.getAdd_source_URL_base();
		if ( Message.isDebugOn ) {
			Message.printDebug( 25, routine,
			"Additional Source directory is: \"" + add_db_source_dir + "\"." );
		}
        	
		//For defining a New ImportProduct
 		__props_tab_add_source_dir_JTextField.setText( add_db_source_dir );
        	
		//get additional source FILE	
		String add_db_source_file = null;
		add_db_source_file = __db_RTi_ImportProduct.getAdd_source_URL_base();
		if ( Message.isDebugOn ) {
			Message.printDebug( 25, routine,
			"Additional source file is: \"" + add_db_source_file + "\"." );
		}
		if( (add_db_source_file==null) || ( add_db_source_file.equalsIgnoreCase("")) ) {
			add_db_source_file = "*";
		}
        	
 		__props_tab_add_source_file_JTextField.setText( add_db_source_file );
	}
...
public void verify_properties_tab() throws Exception {
	...
	// All related to these add_source should also be version dependent	REVISIT
	if( __db_ImportProduct_type.equalsIgnoreCase( __type_diadvisor_str) ) {
		//SOURCE Directory - NOT REQUIRED
		String gui_add_source_dir  = null;
		String gui_add_source_file = null;
		String db_add_source_dir   = null;
		String db_add_source_file  = null;
		db_add_source_dir  = __db_RTi_ImportProduct.getAdd_source_URL_base();
		db_add_source_file = __db_RTi_ImportProduct.getAdd_source_URL_file();
        	
		gui_add_source_dir = ( __props_tab_add_source_dir_JTextField.getText()).trim();
		if ( Message.isDebugOn ) {
			Message.printDebug( 35, routine,
			"Value for File Tab: Source directory: \"" + 
			gui_add_source_dir + "\"." );
		...
		if ( !db_add_source_dir.equalsIgnoreCase( gui_add_source_dir )){
 			...
 			__gui_RTi_ImportProduct.setAdd_source_URL_base( 
				gui_add_source_dir);
		}
        	
		//Source File
		gui_add_source_file = ( __props_tab_add_source_file_JTextField.getText()).trim();
		if ( Message.isDebugOn ) {
			Message.printDebug( 35, routine,
			"Value for File Tab: Additional Source file: \"" + 
			gui_add_source_file + "\"." );
		}
		//compare value to what was originally in db
		if ( !db_add_source_file.equalsIgnoreCase( gui_add_source_file )){
 			...
 			__gui_RTi_ImportProduct.setAdd_source_URL_file( 
				gui_add_source_file);
		}
...
public void actionPerformed (ActionEvent event) {
	...
	if ( source.equals( __automation_tab_automated_JCheckBox )) {
		...
		if ( __bln_new_object ) {
			...
			if ( DMIUtil.isMissing( db_id ) ) {
				...
				try {
					__dmi.deleteImportProductForImportProduct_num((int) __db_ImportProduct_num );
				}
				catch (Exception e ) {
					Message.printWarning( 2, routine, e);
				}
	...
	//BROWSE BUTTONS
	...
	else if ( source.equals( __props_tab_add_source_browse_JButton ) ) {
		String cur_dir = null;
		cur_dir = __props_tab_add_source_dir_JTextField.getText().trim(); 
		if (( cur_dir == null ) || ( cur_dir.length() <= 0 )) {
			cur_dir = JGUIUtil.getLastFileDialogDirectory();
		}

		JFileChooser chooser = JFileChooserFactory.createJFileChooser();
		String open_file_title = "Select Additional Source Directory";
		chooser.setDialogTitle( open_file_title );
		chooser.setFileSelectionMode( 
		JFileChooser.FILES_AND_DIRECTORIES );
		File file = new File( cur_dir );
		...
		if ( returnVal == JFileChooser.APPROVE_OPTION ) {
 			__props_tab_add_source_dir_JTextField.setText( 
			chooser.getSelectedFile().getPath());
		}
		chooser = null;
	...
	if ( source == __archive_tab_wild_JComboBox ) {
	...
	else if ( source == __props_tab_add_source_wild_JComboBox ) {
		//get the wildcard selected and add to textfield
		String wild = null;
		wild = (String) __props_tab_add_source_wild_JComboBox.
		getSelected();
		//remove comments
		int ind = -999;
		ind = wild.indexOf( " -");
		...
		if ( wild.equalsIgnoreCase("*") ) {
 			__props_tab_add_source_file_JTextField.
				setText(wild);
		}
		else if ( wild.equalsIgnoreCase( "*." ) ) {
			String gui_dir = null;
			gui_dir = __props_tab_add_source_file_JTextField.
				getText();
 			__props_tab_add_source_file_JTextField.
				setText( "*." + gui_dir );
		}
