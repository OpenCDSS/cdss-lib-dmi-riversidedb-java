...
//------------------------------------------------------------------------------
// RiversideDB_ReferenceTable_MeasTimeScale_JFrame - Frame extending from
//    RiversideDB_ReferenceTable_Abstract_JFrame and containing the editor for
//    the RiversideDB table MeasTimeScale.
//------------------------------------------------------------------------------
...
the editor for the RiversideDB table MeasTimeScale.
*/
public class RiversideDB_ReferenceTable_MeasTimeScale_JFrame extends
	RiversideDB_ReferenceTable_Abstract_JFrame {

/**
Class name.
*/
public final String CLASS = "RiversideDB_ReferenceTable_MeasTimeScale_JFrame";
...
*/
public RiversideDB_ReferenceTable_MeasTimeScale_JFrame(
		RiversideDB_DMI dmi,
...
{
	// Creates a blank record object for the MeasTimeScale table, marks it
	// as a new, and inserts it into the tabl.
	RiversideDB_MeasTimeScale mts = new RiversideDB_MeasTimeScale();
	mts.setOriginal   ( null );
	_worksheet.addRow (  mts );
...
protected DMIDataObject[] checkForKeyChange(DMIDataObject ddo)
{
	...
	RiversideDB_MeasTimeScale mts = (RiversideDB_MeasTimeScale)
					ddo;
	RiversideDB_MeasTimeScale mts2 = (RiversideDB_MeasTimeScale)
					mts.getOriginal();
	mts2.setOriginal(mts2.cloneSelf());
	if (mts.getScale().trim().equals(mts2.getScale().trim())) {
		return null;
	}
	obs[0] = mts;
	obs[1] = mts2;
...
protected boolean dataObjectsEqual( DMIDataObject d1, DMIDataObject d2 )
{
	boolean eq = false;
		...
		eq = (	(RiversideDB_MeasTimeScale) d1 ).equals(
			(RiversideDB_MeasTimeScale) d2 );
...
throws Exception
{
	...
	RiversideDB_MeasTimeScale mts  = (RiversideDB_MeasTimeScale)
					ddo;
	RiversideDB_MeasTimeScale mts0 = (RiversideDB_MeasTimeScale)
					mts.getOriginal();
	RiversideDB_MeasTimeScale mtsr =
		_dmi.readMeasTimeScaleForScale(mts0.getScale() );

	if ( shouldDelete(
		mts, mts0, mtsr, "Scale", mts.getScale() ) ) {
		_dmi.deleteMeasTimeScaleForScale( mts0.getScale() );
	}
}
...
{
	_worksheet.addRow(new RiversideDB_MeasTimeScale());
...
protected void informWindowManager()
{
	_windowManager.closeWindow(
		_windowManager.WINDOW_REF_MEAS_TIME_SCALE );
}
...
protected void readData()
{
	...
	try {
		_results = _dmi.readMeasTimeScaleList();
	}
	catch ( Exception e ) {
...
protected void setupGUI()
{
	...
	try {
		_model = new
			RiversideDB_ReferenceTable_MeasTimeScale_TableModel (
...
throws Exception
{
	...
	RiversideDB_MeasTimeScale mts0 = (RiversideDB_MeasTimeScale)
					mts.getOriginal();
	RiversideDB_MeasTimeScale mtsr =
		_dmi.readMeasTimeScaleForScale( mts.getScale() );

	if ( shouldWrite(
		mts, mts0, mtsr, "Scale", mts.getScale() ) ) {
		_dmi.writeMeasTimeScale( mts );
	}
}
