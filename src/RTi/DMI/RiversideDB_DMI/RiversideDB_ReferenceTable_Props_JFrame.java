...
//------------------------------------------------------------------------------
// RiversideDB_ReferenceTable_Props_JFrame - Frame extending from
//    RiversideDB_ReferenceTable_Abstract_JFrame and containing the editor for
//    the RiversideDB table Props.
//------------------------------------------------------------------------------
...
the editor for the RiversideDB table Props.
*/
public class RiversideDB_ReferenceTable_Props_JFrame extends
	RiversideDB_ReferenceTable_Abstract_JFrame {

/**
Class name.
*/
public final String CLASS = "RiversideDB_ReferenceTable_Props_JFrame";
...
*/
public RiversideDB_ReferenceTable_Props_JFrame(
		RiversideDB_DMI dmi,
...
{
	// Creates a blank record object for the Props table, marks it
	// as a new, and inserts it into the tabl.
	RiversideDB_Props p = new RiversideDB_Props();
	p.setOriginal    ( null );
	_worksheet.addRow(    p );
...
protected DMIDataObject[] checkForKeyChange( DMIDataObject ddo )
{
	...
	RiversideDB_Props dd  = (RiversideDB_Props)
					ddo;
	RiversideDB_Props dd2 = (RiversideDB_Props)
					dd.getOriginal();
					
	dd2.setOriginal( dd2.cloneSelf() );
	if ( dd.getProp_num() == dd2.getProp_num() ) {
		return null;
	}
	obs[0] = dd;
	obs[1] = dd2;
...
protected boolean dataObjectsEqual( DMIDataObject d1, DMIDataObject d2 )
{
	...
	eq = ( 	(RiversideDB_Props) d1 ).equals(
		(RiversideDB_Props) d2 );
...
throws Exception
{
	RiversideDB_Props worksheetObject  = (RiversideDB_Props)
						ddo;
	RiversideDB_Props originalObject   = (RiversideDB_Props)
						worksheetObject.getOriginal();
	RiversideDB_Props dmiObject        = _dmi.readPropsForVariable(
						 originalObject.getVariable() );
	if ( shouldDelete( worksheetObject,
			   originalObject, 
			   dmiObject, 
			   "Variable", 
			   worksheetObject.getVariable() ) ) {
		_dmi.deletePropsForVariable( originalObject.getVariable() );
	}
}
...
{
	_worksheet.addRow( new RiversideDB_Props() );
...
protected void informWindowManager()
{
	_windowManager.closeWindow(
		_windowManager.WINDOW_REF_PROPS );
}
...
protected void readData()
{
	...
	try {
		_results = _dmi.readPropsListVector();
	}
	catch ( Exception e ) {
...
protected void setupGUI()
{
	...
	try {
		_model = new 
			RiversideDB_ReferenceTable_Props_TableModel (
...
throws Exception
{
	RiversideDB_Props dd  = (RiversideDB_Props)
					ddo;
	RiversideDB_Props dd0 = (RiversideDB_Props)
					dd.getOriginal();
	RiversideDB_Props ddr =
		_dmi.readPropsForVariable(dd.getVariable());

	if ( shouldWrite(
		dd, dd0, ddr, "Variable", dd.getVariable() ) ) {
		_dmi.writeProps( dd );
	}
}
