...
public void backupData()
{
	for (int i = 0; i < _rows; i++) {	
		RiversideDB_MeasQualityFlag m2 = (RiversideDB_MeasQualityFlag)_data.get(i);
...
protected String checkTableField( String s, int row )
{
	...
	for (int i = 0; i < _rows; i++) {
		if (i != row) {
			d = (RiversideDB_MeasQualityFlag)_data.get(i);
...
{
	RiversideDB_MeasQualityFlag m = (RiversideDB_MeasQualityFlag)_data.get(row);
	switch (column) {
...
{
	RiversideDB_MeasQualityFlag m = (RiversideDB_MeasQualityFlag)_data.get(row);
	RiversideDB_MeasQualityFlag m0 = (RiversideDB_MeasQualityFlag)m.getOriginal();
	String s;
	switch (column) {
		case COL_MEAS_QUALITY_FLAG_QUALITY_FLAG:
